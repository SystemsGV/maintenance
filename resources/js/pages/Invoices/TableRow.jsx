...
export default function TableRow({ item }) {
  ...
  return (
    <Table.Tr key={item.id}>
      ...
      {(can("editar factura") || can("archivar factura") || can("restaurar factura")) && (
        <Table.Td>
          <TableRowActions
            item={item}
            editRoute="invoices.edit"
            editPermission="editar factura"
            archivePermission="archivar factura"
            restorePermission="restaurar factura"
            archive={{
              route: "invoices.destroy",
              title: "Archivar factura",
              content: `¿Está seguro de que desea archivar esta factura?`,
              confirmLabel: "Archivar",
            }}
            restore={{
              route: "invoices.restore",
              title: "Restaurar factura",
              content: `¿Está seguro de que desea restaurar esta factura?`,
              confirmLabel: "Restaurar",
            }}
          >
            {can("descargar factura") && (
              <ActionIcon variant="subtle" color="blue" onClick={() => downloadPdf()}>
                <IconFileDownload style={{ width: rem(16), height: rem(16) }} stroke={1.5} />
              </ActionIcon>
            )}

            {can("imprimir factura") && (
              <ActionIcon
                variant="subtle"
                color="blue"
